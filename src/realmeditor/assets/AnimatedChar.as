package realmeditor.assets
{
   ...
   public class AnimatedChar
   {
      ...
      private static const SEC_TO_DIRS:Vector.<Vector.<int>> = new <Vector.<int>>[new <int>[1,3,2],new <int>[3,1,2],new <int>[3,0,2],new <int>[0,3,2],new <int>[0,2],new <int>[2,0],new <int>[2,1],new <int>[1,2]];
      ...
      public function AnimatedChar(image:MaskedImage, width:int, height:int, firstDir:int)
      {
         ...
         if(firstDir == 0)
         {
            this.dict_[0] = this.loadDir(0,false,false,frames);
            this.dict_[1] = this.loadDir(0,true,false,frames);
            if(frames.images_.length >= 14)
            {
               this.dict_[2] = this.loadDir(7,false,true,frames);
               if(frames.images_.length >= 21)
               {
                  this.dict_[3] = this.loadDir(14,false,true,frames);
               }
            }
         }
         else if(firstDir == 2)
         {
            this.dict_[2] = this.loadDir(0,false,true,frames);
            if(frames.images_.length >= 14)
            {
               this.dict_[0] = this.loadDir(7,false,false,frames);
               this.dict_[1] = this.loadDir(7,true,false,frames);
               if(frames.images_.length >= 21)
               {
                  this.dict_[3] = this.loadDir(14,false,true,frames);
               }
      ...
      public function getFirstDirImage() : BitmapData
      {
         ...
         var vec:Vector.<MaskedImage> = actionDict[0];
         if(vec.length > 0)
         {
      ...
      {
         var sec:int = (int(angle / 0.7853981633974483 + 4)) % 8;
