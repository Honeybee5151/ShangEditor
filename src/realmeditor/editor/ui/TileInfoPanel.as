package realmeditor.editor.ui
{
   ...
   public class TileInfoPanel extends Sprite
   {
      ...
      public function setInfo(x:int, y:int, tileData:MapTileData) : void
      {
         ...
         {
            groundId = GroundLibrary.getIdFromType(tileData.groundType);
         ...
         {
            objectId = ObjectLibrary.getIdFromType(tileData.objType);
         ...
         {
            regionId = RegionLibrary.getIdFromType(tileData.regType);
      ...
      {
         var groundText:BitmapData = null;
         var groundMatrix:Matrix = null;
         var objectText:BitmapData = null;
         var objectMatrix:Matrix = null;
         var regColor:* = 0;
         if(this.groundImage)
         {
         ...
         if(tileData.groundType != -1)
         {
            groundText = GroundLibrary.getBitmapData(tileData.groundType);
            groundMatrix = new Matrix();
            groundMatrix.scale(20 / groundText.width,20 / groundText.height);
            this.groundTexture = new BitmapData(20,20,true,0);
         ...
         if(tileData.objType > 0)
         {
            objectText = ObjectLibrary.getTextureFromType(tileData.objType);
            objectMatrix = new Matrix();
            objectMatrix.scale(20 / objectText.width,20 / objectText.height);
            this.objectTexture = new BitmapData(20,20,true,0);
         ...
         if(tileData.regType > 0)
         {
            regColor = uint(RegionLibrary.getColor(tileData.regType));
            this.regionTexture = new BitmapData(20,20,true,0);
            this.regionTexture.fillRect(new Rectangle(0,0,20,20),0x5F000000 | regColor);
