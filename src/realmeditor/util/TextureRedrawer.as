package realmeditor.util
{
   ...
   public class TextureRedrawer
   {
      ...
      public static const OUTLINE_FILTER:GlowFilter = new GlowFilter(0,0.8,1.4,1.4,255,1,false,false);
      ...
      {
         var h:* = (padBottom ? 134217728 : 0) | size * sMult;
         if(glowColor == 0)
         {
            return h;
         }
      ...
      public static function resize(tex:BitmapData, mask:BitmapData, size:int, padBottom:Boolean, op1:int, op2:int, sMult:Number = 5) : BitmapData
      {
         ...
         m.translate(12,12);
         var ret:BitmapData = new BitmapDataSpy(w + 24,h + (padBottom ? 12 : 1) + 12,true,0);
         ret.draw(tex,m);
         return ret;
      }
      ...
      public static function redrawSolidSquare(color:uint, size:int) : BitmapData
      {
         ...
         tex.fillRect(new Rectangle(4,4,size,size),4278190080 | color);
      ...
      private static function getTexture(op:int, bmp:BitmapData) : BitmapData
      {
         var ret:* = null;
         var type:* = op >> 24 & 0xFF;
         var value:uint = uint(op & 0xFFFFFF);
         switch(type)
         {
            case 0:
               ret = bmp;
